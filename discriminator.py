...
def build_discriminator(x_data, x_generated, batch_size, sentence_length, embedding_size):
  """
  ...
  assert(x_in.get_shape().as_list() == [2*batch_size, sentence_length, embedding_size, 1])
  ...
  with tf.variable_scope('conv') as scope:
    ...
    conv1_filter = tf.Variable(tf.random_normal([num_words, embedding_size, 1, num_filters]), name="weights")
    conv1_bias = tf.Variable(tf.random_normal([num_filters]), name="bias") # TODO initialize to zero?
  ...
  with tf.variable_scope('fc') as scope:
    ...
    fc_weights = tf.Variable(tf.random_normal([dim, 2]), validate_shape=False, name="weights")
    fc_bias = tf.Variable(tf.random_normal([2]), name="bias") # TODO initialize to zero?
