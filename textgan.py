...
x_generated_ids, x_generated, g_params = build_generator(
    z_prior, embeddings, num_classes, state_size, embedding_size, prior_size,
    max_sentence_length)
...
if args.d_pretrain_filepath is not None:
    init_d_fn = tf.contrib.framework.assign_from_checkpoint_fn(
        args.d_pretrain_filepath,
        tf.contrib.framework.get_variables_to_restore(include=[
            'discriminator/conv/weights_3', 'discriminator/conv/weights_4',
            'discriminator/conv/weights_5', 'discriminator/conv/bias',
            'discriminator/discriminator_fc_1/weights',
            'discriminator/discriminator_fc_1/bias',
            'discriminator/discriminator_fc_2/weights',
            'discriminator/discriminator_fc_2/bias',
            'discriminator/encoder_fc_1/weights',
            'discriminator/encoder_fc_1/bias',
            'discriminator/encoder_fc_2/weights',
            'discriminator/encoder_fc_2/bias'
        ]))
...
with tf.Session(config=config) as sess:
    ...
    for batch_i, batch in enumerate(batch_gen()):
        ...
        z_value = np.random.normal(
            0, 1, size=(batch_size, prior_size)).astype(np.float32)
